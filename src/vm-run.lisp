...
Otherwise a fresh state is created from OUTPUT-STREAM."
  (let* ((instructions (vm-program-instructions program))
    ...
    (dolist (cell (vm-program-load-time-value-cells program))
      (unless (vm-load-time-value-cell-resolved-p cell)
        (setf (vm-load-time-value-cell-value cell) (eval (vm-load-time-value-cell-form cell))
              (vm-load-time-value-cell-resolved-p cell) t))
      (setf (gethash (vm-load-time-value-cell-id cell) (vm-load-time-values state))
