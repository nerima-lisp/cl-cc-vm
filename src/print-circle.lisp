...
(defun %print-circle-state-has-labels-p (state)
  (plusp (hash-table-count (vm-print-circle-state-%label-objects state))))
...
Uses the host fast path when CIRCLE is NIL; the full circle-detection path otherwise."
  ...
  (if (not circle)
      (%vm-host-write-object-to-string object escape)
      (let ((state (%collect-shared-objects object)))
        (if (%print-circle-state-has-labels-p state)
