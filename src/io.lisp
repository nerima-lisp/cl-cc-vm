...
;;; VM I/O state and stream handles, built on io-external-format.lisp
;;; (loads before): vm-open, with-standard-io-syntax, the vm-io-state CLOS
;;; class, clone-vm-state, vm-string-input-stream, the handle/bridge
;;; machinery (vm-get-stream, vm-allocate-file-handle, %vm-bridge-*), and
;;; with-binary-file.
;;;
;;; Networking (sockets, DNS, TLS, mmap) is in io-network.lisp (loads next).
;;; All define-vm-instruction forms for I/O operations are in
;;; io-instructions.lisp; their execute-instruction methods are in
;;; io-execute.lisp; run-compiled-with-io / run-string-with-io are in
;;; io-runners.lisp.
